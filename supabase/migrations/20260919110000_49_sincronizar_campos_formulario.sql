-- ==============================================================================
-- Migração 49: Sincronização dos campos do formulário com a base de dados
-- ==============================================================================

-- 1. Adicionar colunas em falta na tabela reservas
ALTER TABLE reservas 
ADD COLUMN IF NOT EXISTS opcao_menu TEXT DEFAULT 'com_menu',
ADD COLUMN IF NOT EXISTS extra_cachorro BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS extra_fruta BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS extra_gelatina BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS decoracao_tematica BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS decoracao_tema_nome TEXT,
ADD COLUMN IF NOT EXISTS pinturas_faciais BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS outros_servicos TEXT,
ADD COLUMN IF NOT EXISTS inclui_bolo BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS tema_convite TEXT;

-- 2. Alterar o default da coluna tipo_convite para 'nenhum'
ALTER TABLE reservas ALTER COLUMN tipo_convite SET DEFAULT 'nenhum';

-- 3. Atualizar a RPC atualizar_reserva_b2c para aceitar e persistir todos os campos do frontend
